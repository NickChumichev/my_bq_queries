...
-- QUALIFY ROW_NUMBER() OVER (PARTITION BY date, source, medium, CAST(campaignId AS STRING), CAST(adCost AS STRING) ORDER BY rename DESC) =1
...
FROM old_new_rk
WHERE source = 'google'
QUALIFY ROW_NUMBER() OVER (PARTITION BY date, medium, campaign_id, CAST(cost AS STRING) ORDER BY rename DESC) = 1 
