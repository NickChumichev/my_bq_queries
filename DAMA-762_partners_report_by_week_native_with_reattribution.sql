--partners_report_by_week_native (версия с реатрибуцией)
...
SELECT DISTINCT -- объединить промокоды с данными по покупкам
  ...
  CASE    
    ...
    y.promocode,
    purchase_id,
    IF(acquisition in ('aso_organic', 'seo', 'sms','direct', 'flowwow_com') or acquisition is null, y.purchase_id, NULL) AS purchase_id_reattrib, --покупки с реатрибуцией
    ...
    y.promo_sum_rub AS promo_sum_rub, 
    y.bonus_company AS bonus_company
...
ORDER BY partner
 ...
 , list_of_partners AS ( -- получить партнера и промокод на каждую сгенерированную дату
SELECT DISTINCT
  DATE_TRUNC(date_generate,WEEK(MONDAY)) AS week,
  partner,
  promocode,
  0 AS purchases,
...
ORDER BY partner,promocode,week ASC
 )
...
SELECT -- удалить дубли
  ...
  revenue_without_promo_bonus_reattrib
FROM union_tables
QUALIFY ROW_NUMBER() OVER (PARTITION BY week, partner ORDER BY purchases DESC) = 1
ORDER BY partner,week DESC
)
, real_begin_week AS (
SELECT -- получить неделю начала покупок
  ...
  partner,
  purchases,
  FIRST_VALUE(week) OVER (PARTITION BY partner ORDER BY week rows between unbounded preceding and unbounded following) AS real_begin_week
FROM union_tables
WHERE 1=1
AND purchases != 0
ORDER BY partner,week DESC
...
LEFT JOIN real_begin_week k ON s.week = k.week AND s.partner = k.partner
QUALIFY s.week >= MAX(k.real_begin_week) OVER (PARTITION BY s.partner)
ORDER BY partner, week DESC