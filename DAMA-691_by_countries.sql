WITH a AS ( -- фиксированный расход по интеграции
  ...
  SELECT --расходы по промокодам
    ...
    CASE --страна расхода по промокоду
      WHEN country_from IN ('Испания') THEN 'Испания' 
      WHEN country_from IN ('Сербия') THEN 'Сербия'
      WHEN country_from IN ('Турция') THEN 'Турция'
      WHEN country_from IN ('Грузия') THEN 'Грузия'
      WHEN country_from IN ('Казахстан') THEN 'Казахстан'
      WHEN country_from IN ('Беларусь') THEN 'Беларусь'
      WHEN country_from IN ('Объединенные Арабские Эмираты') THEN 'Объединенные Арабские Эмираты'
      ELSE 'other countries'
      END AS country_from,
  ...
  SELECT -- транзакции по интеграциям без промокодов
    ...
    CASE   --страна расхода по промокоду
      WHEN s.country_from IN ('Испания') THEN 'Испания' 
      WHEN s.country_from IN ('Сербия') THEN 'Сербия'
      WHEN s.country_from IN ('Турция') THEN 'Турция'
      WHEN s.country_from IN ('Грузия') THEN 'Грузия'
      WHEN s.country_from IN ('Казахстан') THEN 'Казахстан'
      WHEN s.country_from IN ('Беларусь') THEN 'Беларусь'
      WHEN s.country_from IN ('Объединенные Арабские Эмираты') THEN 'Объединенные Арабские Эмираты'
      ELSE 'other countries'
      END AS country_from, 
