...
SELECT
  ...
  d.category_name,
  d.purchase_month,
  d.repeated_purchaser,
  d1.repeated_purchaser_without_categories,
  ...
  d1.repeated_purchaser_without_categories * SAFE_DIVIDE(d.repeated_purchaser , SUM(d.repeated_purchaser) OVER (PARTITION BY d.month_of_registration,d.purchase_month, d.country_by_phone)) AS repeated_purchaser_coef,
  d.repeated_purchase,
  d1.repeated_purchase_without_categories,
  SAFE_DIVIDE(d.repeated_purchase , SUM(d.repeated_purchase) OVER (PARTITION BY d.month_of_registration,d.purchase_month, d.country_by_phone)) AS repeated_purchase_proportion,
   d1.repeated_purchase_without_categories * SAFE_DIVIDE(d.repeated_purchase , SUM(d.repeated_purchase) OVER (PARTITION BY d.month_of_registration,d.purchase_month, d.country_by_phone)) AS repeated_purchase_coef,
  repeated_revenue
FROM d LEFT JOIN d1 USING(month_of_registration,country_by_phone,purchase_month)  
WHERE country_by_phone = 'Россия'
ORDER BY month_of_registration, country_by_phone ASC
)
, e AS ( --соединить покупки по годам и категориям с регистрациями
SELECT
  b.month_of_registration,
  b.country_by_phone,
  c.category_name,
...
, e1 AS ( --соединить покупки по годам с регистрациями
SELECT
  b.month_of_registration,
  b.country_by_phone,
  FORMAT_DATE("%Y",c.purchase_month) AS purchase_year,
...
, e2 AS ( --посчитать повторные покупки и покупателей без дублей
SELECT
  e.month_of_registration,
  e.country_by_phone,
  e.category_name,
  e.purchase_year,
  e.repeated_purchaser,
  e1.repeated_purchaser_without_categories,
  SAFE_DIVIDE(e.repeated_purchaser , SUM(e.repeated_purchaser) OVER (PARTITION BY e.month_of_registration,e.purchase_year, e.country_by_phone)) AS repeated_purchaser_proportion,
  e1.repeated_purchaser_without_categories * SAFE_DIVIDE(e.repeated_purchaser , SUM(e.repeated_purchaser) OVER (PARTITION BY e.month_of_registration,e.purchase_year, e.country_by_phone)) AS repeated_purchaser_coef,
  e.repeated_purchase,
  e1.repeated_purchase_without_categories,
  SAFE_DIVIDE(e.repeated_purchase , SUM(e.repeated_purchase) OVER (PARTITION BY e.month_of_registration,e.purchase_year, e.country_by_phone)) AS repeated_purchase_proportion,
  e1.repeated_purchase_without_categories * SAFE_DIVIDE(e.repeated_purchase , SUM(e.repeated_purchase) OVER (PARTITION BY e.month_of_registration,e.purchase_year, e.country_by_phone)) AS repeated_purchase_coef,
  repeated_revenue
FROM e LEFT JOIN e1 USING(month_of_registration,country_by_phone,purchase_year)  
WHERE country_by_phone = 'Россия'
ORDER BY month_of_registration, country_by_phone ASC
)
