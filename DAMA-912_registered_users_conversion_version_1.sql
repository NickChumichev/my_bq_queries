--country_by_phone понять как null появился, т.к phonecode не нашёлся в словаре
--purchasers_country_to_by_phone + purchasers_country_to_no_by_phone не дают purchasers, т.к у user_id может быть оба случая
-- purchase_country_by_phone_rf = purchasers ,т.к в country_by_phone Россия значится
...
-- AND _purchases.country_to != _purchases.country_from
-- QUALIFY ROW_NUMBER() OVER (PARTITION BY _purchases.user_id) > 10
-- AND user_id = 5299331 
)
...
-- WHERE purchases.user_id = 5329938
)
SELECT
  country_by_phone,
  ...
  COUNT(DISTINCT IF(first_purchase_country_to_by_phone IS NOT NULL,purchaser,NULL)) AS first_purchaser_country_to_by_phone,
  ...
  COUNT(DISTINCT IF(purchase_country_by_phone_rf IS NOT NULL,purchaser,NULL)) AS purchase_country_by_phone_rf,
