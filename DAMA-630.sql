...
SELECT DISTINCT 
  ...
  IF(campaign_id NOT IN (0), LAST_VALUE(LOWER(campaign_info)) OVER (PARTITION BY campaign_id ORDER BY date ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING),campaign) AS campaign_info,
  cost,
  clicks,
  impressions
FROM costs
...
SELECT
  ...
  a.campaign,
  a.campaign_info,
  -- IF(b.new_campaign IS NOT NULL,b.new_campaign,a.campaign) AS campaign,
  -- IF(b.new_campaign_info IS NOT NULL,b.new_campaign_info,a.campaign_info) AS campaign_info,
  SUM(a.cost) AS cost
FROM campaign_last_name AS a
-- LEFT JOIN `funnel-flowwow.GOOGLE_SHEETS.CGS_CAMPAIGN_DICTIONARY` b ON a.campaign_info = b.old_campaign_info
WHERE REGEXP_CONTAINS(LOWER(a.campaign),r'seva_wrld_customer_apppromo_ios14_othercountries_purchase_aem\|id-23861857273250492')
-- AND campaign_info != 'flowwow_customer_global_actualseva_wrld_customer_apppromo_ios14_othercountries_purchase_aem|id-23861857273250492|cat-1|subc-0|regionid-99|cityid-nocity|geo_type-global|pl-iosseva_wrld_customer_apppromo_ios14_othercountries_ultraactive_purchase_feed_aem_7dattr|id-23862222435130492|cat-1|subc-0|regionid-99|cityid-2204899|geo_type-local|pl-iosfeed'
GROUP BY 1,2,3