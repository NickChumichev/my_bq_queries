...
SELECT -- определить страну по номеру
  ...
  CASE
    ...
    WHEN STARTS_WITH(phone,"375")THEN "Беларусь" 
    ...
    WHEN STARTS_WITH(phone,"381")THEN "Сербия"
...
SELECT DISTINCT 
  ...
  IF(category_name IS NULL,NULL,purchase_id) AS purchase_id,
  -- CASE 
  --   WHEN category_name IS NULL AND is_first_purchase = 1 THEN NULL
  --   WHEN  category_name IS NOT NULL AND is_first_purchase = 1 THEN purchase_id
  -- END AS first_purchase,
  -- CASE
  --   WHEN category_name IS NULL AND is_first_purchase = 0 THEN NULL
  --   WHEN  category_name IS NOT NULL AND is_first_purchase = 0 THEN purchase_id
  -- END AS repeated_purchase,
...
SELECT
  ...
  _d.category_name,
  _d1.purchase_month,
  0 AS repeated_purchaser_proportion,
  0 AS repeated_purchaser,
  0 AS repeated_purchase_proportion,
  0 AS repeated_purchase,
  0 AS first_purchase,
  0 AS repeated_revenue
FROM d AS _d FULL JOIN d1 AS _d1 USING(month_of_registration) --получить все category_name,которые падают в purchase_month != month_of_registration
)
...
-- AND _c3.category_name != 'no_category'
...
FROM f1 AS _f1
-- WHERE _f1.category_name != 'no_category'
)
...
FROM b AS _b4
LEFT JOIN c AS _c4
ON _b4.registered_user = _c4.purchaser AND _b4.month_of_registration = _c4.purchase_month
-- WHERE month_of_registration = '2023-01-01' AND country_by_phone = 'Россия'
GROUP BY month_of_registration,country_by_phone,category_name
)
, h1 AS ( -- количество регистраций по месяцам и категориям
SELECT DISTINCT
  _b5.month_of_registration, -- первая запись о пользователи
--   _c5.purchase_month,
  _b5.country_by_phone,
  COUNT(DISTINCT _b5.registered_user) AS registered_user
FROM b AS _b5
LEFT JOIN c AS _c5
ON _b5.registered_user = _c5.purchaser AND _b5.month_of_registration = _c5.purchase_month
-- WHERE month_of_registration = '2023-01-01' AND country_by_phone = 'Россия'
-- AND (purchase_month = '2023-01-01' OR NULL)
GROUP BY 1,2
)
...
SELECT
  ...
  _h.country_by_phone,
  IF(_h.category_name IS NULL, 'no_category', _h.category_name) AS category_name,
...
-- WHERE _c6.category_name != 'no_category'
GROUP BY month_of_registration,country_by_phone,category_name
)
...
WHERE _c7.category_name != 'no_category'
...
LEFT JOIN i1 AS _i1
USING(month_of_registration,country_by_phone) 
-- WHERE _i.category_name != 'no_category' 
ORDER BY month_of_registration, country_by_phone ASC
)
...
-- WHERE _c8.category_name != 'no_category'
)
...
-- WHERE _c9.category_name != 'no_category'
...
-- WHERE _c10.category_name != 'no_category'
...
SELECT
  ...
  first_purchases,
  repeated_purchases_products_proportion,
  IF(category_name = 'no_category',month_of_registration,purchase_period) AS purchase_period,
  repeated_purchaser,
  repeated_purchases,
  repeated_revenue
FROM sub_7 AS _sub_7
-- WHERE purchase_period IS NOT NULL AND registered_user IS NOT NULL
-- AND month_of_registration = '2023-01-01' AND country_by_phone = 'Россия'
)
, sub_9 AS ( --покупки в первом месяце без категорий по когортам
SELECT
  month_of_registration,
  country_by_phone,
  IF(category_name = 'no_category',month_of_registration,purchase_period) AS purchase_period,
  SUM(purchaser_first_month) AS purchaser_first_month
FROM sub_7 AS _sub_8
-- WHERE purchase_period IS NOT NULL AND registered_user IS NOT NULL
-- AND month_of_registration = '2023-01-01' AND country_by_phone = 'Россия'
GROUP BY month_of_registration, country_by_phone, purchase_period
)
, sub_10 AS ( -- уравнять registered_users c категориями и purchaser_first_month
SELECT 
  _sub_8.month_of_registration,
  _sub_8.country_by_phone,
  _sub_8.category_name,
  _sub_8.registered_user AS registered_user_old,
  _sub_9.purchaser_first_month *SAFE_DIVIDE(_sub_8.registered_user,SUM(IF(_sub_8.category_name != 'no_category',_sub_8.registered_user,NULL)) OVER (PARTITION BY _sub_8.month_of_registration,_sub_8.country_by_phone,_sub_8.purchase_period)) AS registered_user_new,
  _sub_8.purchaser_first_month,
  _sub_8.first_month_purchases,
  _sub_8.first_purchases,
  _sub_8.repeated_purchases_products_proportion,
  _sub_8.purchase_period,
  _sub_8.repeated_purchaser,
  _sub_8.repeated_purchases,
  _sub_8.repeated_revenue
FROM sub_8 AS _sub_8 LEFT JOIN sub_9 AS _sub_9 USING(month_of_registration,country_by_phone,purchase_period)
)
SELECT --перераспредить registered_users без категорий
  month_of_registration,
  country_by_phone,
  category_name,
  CASE
    WHEN category_name = 'no_category' THEN (SUM(registered_user_old) OVER (PARTITION BY month_of_registration,purchase_period,country_by_phone) -  SUM(registered_user_new) OVER (PARTITION BY month_of_registration,purchase_period,country_by_phone)) + (IF(category_name = 'no_category',registered_user_new,NULL))
    ELSE registered_user_new
    END AS registered_user,
  purchaser_first_month,
  first_month_purchases,
  first_purchases,
  repeated_purchases_products_proportion,
  ...
  repeated_purchases,
  repeated_revenue
FROM sub_10 AS _sub_10
WHERE category_name IS NOT NULL AND purchase_period IS NOT NULL