...
SELECT
  ...
  -- subcategory,
  purchase_period,
  -- FIRST_VALUE(registered_user) OVER (PARTITION BY year_of_registration ORDER BY purchase_period ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS registered_user_0_period,
  SUM(registered_user) AS registered_user,
  SUM(purchaser_first_year) AS purchaser_first_year,
  SUM(first_year_purchases) AS first_year_purchases,
  -- SUM(first_purchases) AS first_purchases,
  -- SUM(repeated_purchases_products_proportion) AS repeated_purchases_products_proportion,
  -- SUM(repeated_purchaser) AS repeated_purchaser,
  -- SUM(repeated_purchases) AS repeated_purchases,
  -- SUM(repeated_revenue) AS repeated_revenue
FROM l1
WHERE year_of_registration = '2015-01-01' AND country_by_phone = 'Россия'
GROUP BY 1,2,3
)
SELECT
  l1.year_of_registration,
  l1.country_by_phone,
  platform,
  category_name,
  subcategory,
  l1.purchase_period,
  FIRST_VALUE(l2.registered_user) OVER (PARTITION BY l2.year_of_registration ORDER BY l2.purchase_period ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS registered_user_0_period,
  SAFE_DIVIDE(l1.registered_user, FIRST_VALUE(l2.registered_user) OVER (PARTITION BY l2.year_of_registration ORDER BY purchase_period ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)) AS registered_user_0_proportion,
  -- FIRST_VALUE(l2.registered_user) OVER (PARTITION BY l2.year_of_registration ORDER BY l2.purchase_period ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) * SAFE_DIVIDE(l1.registered_user, FIRST_VALUE(l2.registered_user) OVER (PARTITION BY l2.year_of_registration ORDER BY purchase_period ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)) AS registered_user_0_period,
  l1.registered_user,
  l1.purchaser_first_year,
  -- FIRST_VALUE(purchaser_first_year) OVER (PARTITION BY year_of_registration,country_by_phone,platform,category_name,subcategory ORDER BY purchase_period ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS purchaser_first_year_0_period,
  l1.first_year_purchases,
  -- FIRST_VALUE(first_year_purchases) OVER (PARTITION BY year_of_registration,country_by_phone,platform,category_name,subcategory ORDER BY purchase_period ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS first_year_purchases_0_period,
  first_purchases,
  repeated_purchases_products_proportion,
  repeated_purchaser,
  repeated_purchases,
  repeated_revenue
FROM l1 LEFT JOIN l2 USING(year_of_registration,purchase_period)
WHERE l1.year_of_registration = '2015-01-01' AND l1.country_by_phone = 'Россия'