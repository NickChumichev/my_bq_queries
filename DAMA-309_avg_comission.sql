...
FROM `funnel-flowwow.MYSQL_EXPORT.f_shop` p 
  ...
  LEFT JOIN `funnel-flowwow.MYSQL_EXPORT.city_all` l ON p.city_id = l.city_id
  LEFT JOIN `funnel-flowwow.Analyt_ChumichevN.f_holding_legal_ru` u ON p.id = u.holding_id 
WHERE t.range_group_id IN (1,5,3,17)
AND l.city_id IN (2,49,60,151)-- категория цветы и подарки, кондитерка, живые растения, декор
)
SELECT
  b.city,
  b.category_name,
  ...
  (SUM(commission)/4) AS avg_commission -- средняя комиссия от одного магазина
