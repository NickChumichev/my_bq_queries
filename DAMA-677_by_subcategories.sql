...
SELECT DISTINCT
  ...
  COUNT(IF(c.repeated_purchase !=0,c.product_id,NULL)) OVER (PARTITION BY b.month_of_registration,c.platform,c.purchase_month,c.category_name, c.subcategory, b.country_by_phone) AS repeated_products_by_categories, --количество повторно купленных товаров по категориям
...
SELECT  
  ...
  repeated_purchases_products_proportion
FROM f2
GROUP BY month_of_registration,repeated_category_month,platform,country_by_phone,category_name, subcategory, repeated_purchases_products_proportion
)
, h AS ( -- количество регистраций по месяцам, платформам, категориям, подкатегориям
SELECT
  b.month_of_registration,
  b.country_by_phone,
  c.platform,
...
, h1 AS ( -- количество регистраций по месяцам
SELECT
  b.month_of_registration,
  b.country_by_phone,
  COUNT(DISTINCT registered_user) AS registered_user
...
WHERE month_of_registration = '2023-01-01' AND country_by_phone = 'other country'
ORDER BY month_of_registration, country_by_phone ASC
)
...
SELECT DISTINCT
  ...
  c.subcategory AS first_month_subcategories,
  COUNT(IF(b.month_of_registration = c.purchase_month,product_id,NULL)) OVER (PARTITION BY b.month_of_registration, c.platform, c.purchase_month,c.category_name,c.subcategory,b.country_by_phone) AS products_by_categories,
...
GROUP BY b.month_of_registration, b.country_by_phone, c.platform, c.category_name, c.subcategory
)
...
FROM d2
GROUP BY  month_of_registration,purchase_month,platform,country_by_phone,category_name,subcategory
)
...
USING (month_of_registration, purchase_period, platform, country_by_phone,category_name,subcategory)
...
FROM i2)
USING (month_of_registration, platform,country_by_phone,category_name,subcategory)
...
USING (month_of_registration, platform, country_by_phone,category_name,subcategory)
ORDER BY month_of_registration, platform, country_by_phone, category_name, subcategory,purchase_period ASC