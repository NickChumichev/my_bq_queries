...
SELECT DISTINCT 
  ...
  CASE 
    ...
    WHEN platform IN ('web android', 'web ios', 'desktop') THEN 'web'
    ELSE 'underfined'
  END AS platform,
...
, d AS ( --соединить покупки по месяцам, категориям, платформам с регистрациями
SELECT
  b.month_of_registration,
  b.country_by_phone,
  c.platform, --задвоение
  c.category_name, --задвоение
  ...
  COUNT(DISTINCT IF(c.first_purchase !=0,c.first_purchase,NULL)) AS first_purchase,
...
GROUP BY purchase_month,country_by_phone,month_of_registration,category_name,platform
ORDER BY month_of_registration, platform, country_by_phone ASC
)
...
SELECT
  ...
  b.country_by_phone,
  c.purchase_month AS purchase_month,
  COUNT(DISTINCT IF(c.first_purchase !=0,c.first_purchase,NULL)) AS  first_purchase_without_categories,
...
SELECT
  ...
  d1.repeated_purchaser_without_categories * SAFE_DIVIDE(d.repeated_purchaser , SUM(d.repeated_purchaser) OVER (PARTITION BY d.month_of_registration,d.purchase_month, d.country_by_phone)) AS repeated_purchaser, --расхождение из-за этого получается
  ...
  d1.first_purchase_without_categories * SAFE_DIVIDE(d.first_purchase , SUM(d.first_purchase) OVER (PARTITION BY d.month_of_registration, d.purchase_month, d.country_by_phone)) AS first_purchase,
  repeated_revenue
...
SELECT DISTINCT
  ...
  COUNT(IF(c.repeated_purchase !=0,c.product_id,NULL)) OVER (PARTITION BY b.month_of_registration,c.platform,c.purchase_month,c.category_name, b.country_by_phone) AS repeated_products_by_categories, --количество повторно купленных товаров по категориям
...
-- SELECT
--   SUM(repeated_products_by_categories) AS repeated_products_by_categories,
--   SUM(repeated_products_without_categories) AS repeated_products_without_categories,
-- FROM f1
...
SELECT  
  ...
  repeated_purchases_products_proportion
FROM f2
GROUP BY month_of_registration,repeated_category_month,platform,country_by_phone,category_name,repeated_purchases_products_proportion
)
, h AS ( -- количество регистраций по месяцам, платформам, категориям
SELECT
  b.month_of_registration,
  b.country_by_phone,
  c.platform,
...
FROM b LEFT JOIN c ON b.registered_user = c.purchaser
GROUP BY month_of_registration,country_by_phone,platform,category_name
)
, h1 AS ( -- количество регистраций по месяцам, платформам, категориям
SELECT
  b.month_of_registration,
  b.country_by_phone,
  COUNT(DISTINCT registered_user) AS registered_user
...
, i2 AS ( -- покупки первого месяца без дублей
SELECT
  i.month_of_registration,
  i.country_by_phone,
  i.platform,
  i.category_name,
  SAFE_DIVIDE(i.first_month_purchases , SUM( i.first_month_purchases) OVER (PARTITION BY  i.month_of_registration, i.country_by_phone)) AS first_month_purchases_proportion,
...
SELECT DISTINCT
  b.month_of_registration,
  b.country_by_phone,
  c.platform AS first_month_platforms,
  c.category_name AS first_month_categories,
  COUNT(IF(b.month_of_registration = c.purchase_month,product_id,NULL)) OVER (PARTITION BY b.month_of_registration, c.platform, c.purchase_month,c.category_name, b.country_by_phone) AS products_by_categories,
...
GROUP BY b.month_of_registration, b.country_by_phone, c.platform, c.category_name
)
...
SELECT DISTINCT
  ...
  first_month_purchases, --покупки в первом месяце
  first_purchase AS first_purchases,
  repeated_purchases_products_proportion,
  purchase_period,
  repeated_purchaser,
  repeated_purchase AS repeated_purchases,
  repeated_revenue,
FROM 
...
FROM d2
GROUP BY  month_of_registration,purchase_month,platform,country_by_phone,category_name
)
...
USING (month_of_registration, purchase_period, platform, country_by_phone,category_name)
...
FROM i2)
USING (month_of_registration, platform,country_by_phone,category_name)
...
(SELECT
  month_of_registration,
  first_month_categories AS platform,
  country_by_phone,
...
FROM k2)
USING (month_of_registration, platform, country_by_phone,category_name)
ORDER BY month_of_registration, platform, country_by_phone, category_name, purchase_period ASC
)
SELECT
  SUM(first_purchases) AS first_purchases,
  SUM(repeated_purchaser) AS repeated_purchaser,
  SUM(repeated_purchases) AS repeated_purchases,
  SUM(repeated_revenue) AS revenue
FROM l