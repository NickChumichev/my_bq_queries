...
SELECT
  ...
  CASE
    WHEN segment NOT IN ("all_cashback", "creatives", "offline_merch", "pr_international", "pr_rus", "donate", "serm", "snapchat_global_smm") THEN SAFE_DIVIDE(traffic,SUM(traffic) OVER(a))*(SUM(not_ads_cost) OVER(a))
    WHEN segment IN ("all_cashback", "creatives", "offline_merch", "pr_international", "pr_rus", "donate", "serm", "snapchat_global_smm") THEN not_ads_cost
    ELSE null
    END not_ads_cost_by_traffic,
...
SELECT
  ...
  SUM(not_ads_cost_by_traffic) AS not_ads_cost_by_traffic,
  SUM(not_ads_cost) AS not_ads_cost
FROM b RIGHT JOIN c USING(segment)