...
FROM phone_num AS _phone_num LEFT JOIN `funnel-flowwow.GOOGLE_SHEETS.CGS_COUNTRY_PHONE_CODES` AS phone_codes ON STARTS_WITH(_phone_num.phone,phone_codes.phonecode)
)
...
SELECT DISTINCT 
  ...
  IF(crm.country_to = _country_by_phone_num.country_by_phone AND crm.country_to = crm.country_from AND _country_by_phone_num.country_by_phone != 'Россия'AND _no_duplicates.language != 'ru', purchase_id, NULL) AS local_purchase, --локальная покупка
