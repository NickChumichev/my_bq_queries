...
SELECT
  ...
  CASE
    WHEN REGEXP_CONTAINS(LOWER(campaign),"rename") THEN REGEXP_EXTRACT(campaign, r"rename-([0-9]+)")
    ELSE null
    END AS rename,
SUM(cost) as cost,
SUM(clicks) as clicks,
...
SELECT DISTINCT 
  ...
  clicks,
  impressions,
 ROW_NUMBER() OVER (PARTITION BY date, source, medium, campaign_id, CAST(cost AS STRING) ORDER BY rename DESC) AS row_num
FROM old_new_rk
QUALIFY row_num = 1 