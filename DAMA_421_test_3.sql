...
SELECT
  ...
  c.purchase_year AS purchase_year,
...
SELECT
  ...
  COUNT(DISTINCT IF(c.repeated_purchase !=0,c.repeated_purchase,NULL)) AS  repeated_purchase_without_categories, 
FROM b LEFT JOIN c ON b.registered_user = c.purchaser 
GROUP BY purchase_year,country_by_phone,month_of_registration
ORDER BY month_of_registration, country_by_phone ASC
)
...
SELECT
  ...
  e1.repeated_purchaser_without_categories * SAFE_DIVIDE(e.repeated_purchaser , SUM(e.repeated_purchaser) OVER (PARTITION BY e.month_of_registration,e.purchase_year, e.country_by_phone)) AS repeated_purchaser,
  ...
  e1.repeated_purchase_without_categories * SAFE_DIVIDE(e.repeated_purchase , SUM(e.repeated_purchase) OVER (PARTITION BY e.month_of_registration,e.purchase_year, e.country_by_phone)) AS repeated_purchase,
  repeated_revenue
...
SELECT DISTINCT
  b.month_of_registration,
  c.purchase_month AS repeated_category_year,
  b.country_by_phone,
  c.category_name,
...
FROM d2
WHERE purchase_month < "2024-01-01"-- считать по месяцам до 2024-01
...
SELECT -- посчитать покупки по годам
  ...
  SUM(repeated_revenue) AS repeated_revenue
FROM e2
WHERE purchase_year > "2023-12-01" --считать с 2024 года
GROUP BY  month_of_registration,purchase_year,country_by_phone,category_name
)
...
(
  ...
  repeated_purchases_products_proportion
FROM f3
WHERE repeated_category_month < "2024-01-01"
...
SELECT
  ...
  repeated_purchases_products_proportion
FROM g3
WHERE repeated_category_year > "2023-12-01"
) 
