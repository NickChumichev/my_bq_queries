...
WHERE TIMESTAMP_TRUNC(_PARTITIONTIME, DAY) >= TIMESTAMP("2024-05-01")
...
SELECT
  ...
  COUNT(DISTINCT IF(local_purchase IS NOT NULL, purchase_id, NULL)) AS local_purchases,
  COUNT(DISTINCT IF(foreign_purchase IS NOT NULL, purchase_id, NULL)) AS foreign_purchases,
  COUNT(DISTINCT IF(russian_purchase IS NOT NULL, purchase_id, NULL)) AS russian_purchases,
  COUNT(DISTINCT IF(russian_purchase_in_country_from IS NOT NULL, purchase_id, NULL)) AS russian_purchase_in_country_from,
  COUNT(DISTINCT IF(local_purchase IS NULL AND foreign_purchase IS NULL AND russian_purchase IS NULL AND russian_purchase_in_country_from IS NULL, purchase_id, NULL)) AS other_purhases,
...
SELECT
  ...
  ROUND(g.russian_purchases * SAFE_DIVIDE(f.russian_purchases , SUM(f.russian_purchases) OVER (PARTITION BY f.year, f.month, f.country_by_phone, f.country_from, f.country_to, f.platform, f.segment)),0) AS russian_purchases,
   ROUND(g.russian_purchase_in_country_from * SAFE_DIVIDE(f.russian_purchase_in_country_from , SUM(f.russian_purchase_in_country_from) OVER (PARTITION BY f.year, f.month, f.country_by_phone, f.country_from, f.country_to, f.platform, f.segment)),0) AS russian_purchase_in_country_from,
  ROUND(g.other_purhases * SAFE_DIVIDE(f.other_purhases , SUM(f.other_purhases) OVER (PARTITION BY f.year, f.month, f.country_by_phone, f.country_from, f.country_to, f.platform, f.segment)),0) AS other_purchases,
